...
describe("Telemetry analytics model storage layout", () => {
  ...
  test("enables cold-tier while keeping app-facing telemetry table names stable", () => {
    ...
    expect(baseline.getReadTableName()).toBe("MetricBaselineHourly");
    expect(baseline.getWriteTableName()).toBe("MetricBaselineHourly");
    expect(baseline.getMutationTableName()).toBe("MetricBaselineHourly");
    expect(baseline.distributedTableName).toBeUndefined();
    expect(baseline.distributedClusterName).toBeUndefined();
    expect(baseline.distributedShardingKey).toBeUndefined();
