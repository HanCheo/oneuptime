...
The Device Offline template fires when a device reports `iot_device_up = 0`. Three common ways a silent or dead device trips it:

- **Gateway-reported**: the gateway or collector publishes `iot_device_up = 0` for devices it can no longer reach.
- **MQTT Last Will**: devices connecting over OneUptime's MQTT endpoint can register a Last Will on their `status` topic. If the device dies, the broker publishes `iot_device_up = 0` on its behalf the moment the session drops. See [Sending Metrics via MQTT](/docs/telemetry/iot-devices).
- **Registered devices**: devices registered under the fleet's **Device Registry** tab are treated as expected. When a registered device produces no data in the evaluation window, the monitor synthesizes an empty per-device series so the Device Offline template's no-data policy can evaluate it as zero and create one incident per silent device.

An unregistered device that simply vanishes without a Last Will or gateway report stops producing series data and cannot be singled out by a per-device grouped monitor. Register devices you expect to monitor individually, or have the gateway publish `iot_device_up = 0` when a device misses its heartbeat.

Registered-device silent detection needs at least one other device in the fleet still reporting. A fully dark fleet is treated as a telemetry-source blackout and holds monitor state rather than creating one incident per known device. The Fleet Health templates close the fleet-level gap: the rollup worker tracks each device's liveness server-side and counts a device that stopped reporting as stale or offline, so `iot_fleet_online_ratio` still drops even when the device itself pushes nothing.
...
| `iot_fleet_battery_percent_p10` | 10th-percentile battery (%) — the fleet's weakest batteries. Only emitted while fresh readings exist |
...
If you build criteria from scratch, remember to also add a **healthy** criteria (one that matches when values are back to normal and sets the operational monitor status) — without it, the monitor status will not flip back and auto-resolve will never trigger.
...
3. A device that only sends `iot_device_info` shows up in the fleet inventory but reports no data metrics — the Device Offline template needs either an actual `iot_device_up = 0` datapoint or registration-backed silent detection to fire.

### Alerts are not resolving

1. Auto-resolve requires the healthy criteria to **match**, which requires data: the device must report values on the healthy side of the threshold.
